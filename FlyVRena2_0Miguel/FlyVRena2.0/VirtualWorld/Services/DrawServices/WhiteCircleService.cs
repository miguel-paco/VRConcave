...
{
    public class WhiteCircleService : DrawService
    {
        public int np;
        public WhiteCircleService(IServiceContainer wObj, VirtualWorld VW, int circleRadius, int np) : base(wObj, VW)
        {
            this.np = np;
            if (VW.render != null && circleRadius != 0)
            {
                VW.render.ShaderProgramList.TryGetValue("solidProgram", out sp);
                rObj = new ColoredObject(VertexFactory.CreateSolidCircle(circleRadius, np, new float[] { 0, 0 }, 1, Color4.White), sp.Id);
