...
{
    public class WhiteSquareService : DrawService
    {
        public WhiteSquareService(IServiceContainer wObj, VirtualWorld VW, int squareSize) : base(wObj, VW)
        {
            if (VW.render != null && squareSize != 0)
            {
                VW.render.ShaderProgramList.TryGetValue("solidProgram", out sp);
                rObj = new ColoredObject(VertexFactory.CreateSolidSquare(squareSize, 1, Color4.White), sp.Id);
